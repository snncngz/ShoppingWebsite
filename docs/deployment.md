...
Use `npm ci --include=dev`. Render sets `NODE_ENV=production` during build, so a plain `npm ci` skips `devDependencies` and `next build` fails (missing Tailwind, TypeScript, or `@types/*`). The repo also has `.npmrc` (`include=dev`) and keeps Tailwind/TypeScript/`@types` in `dependencies` so a plain `npm ci` can still compile. **Start Command** stays `npm run start`.
